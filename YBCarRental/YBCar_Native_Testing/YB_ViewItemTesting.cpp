...
namespace YBCarTesting
{
	...
	{
	public:
		TEST_METHOD(View_Instantiate_Test)
		{
			int w = 64, h = 24;
			YB_ViewBasis view = YB_ViewBasis(&w, &h);
			view.Id = 999;
			const char* source = "Hello Car Rental";
			...
			Assert::AreEqual("Id:999;Title:Hello Car Rental;w:64;h:24;", serial.c_str());
		}
		TEST_METHOD(ViewItem_Instantiate_Test)
		{
		...
		TEST_METHOD(View_Deserialize_Test)
		{
			int w = 120, h = 30;
			YB_ViewBasis item = YB_ViewBasis(&w, &h);
			string line = "Id:999;Title:Hello Car Rental;w:120;h:30;";
			item.Deserialize(line);
			Assert::AreEqual(30, item.h);
			Assert::AreEqual(120, item.w);
			Assert::AreEqual(999, item.Id);
			Assert::AreEqual("Hello Car Rental", item.Title);
		}
		TEST_METHOD(ViewItem_Deserialize_Test)
		{
